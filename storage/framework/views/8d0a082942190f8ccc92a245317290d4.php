...
<html lang="hu">
    ...
    <?php endif; ?>
		...
		?>
        <?php if($enableNotifications === 0): ?>
		<div class="row bg-danger text-white">
			<div class="col-12 p-1">
        		<?php echo e(__('all.global_settings.notifications_off_warning')); ?> <i>enable-notifications</i> <?php echo e(__('all.global_settings.option')); ?> <a href=<?php echo e(url("globalsettings")); ?> class="text-light font-weight-bold" target="_blank"><?php echo e(__('all.global_settings.global_settings')); ?></a>.
			</div>
		</div>
        <?php endif; ?>
        <?php if(!$isValidFormat): ?>
		<div class="row bg-warning">
			<div class="col-12 p-1">
        		<?php echo e(__('all.layout.env_master_key_warning')); ?>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><?php echo e(__("all.layout.find_workstation")); ?></h5>
	  </div>
      <div class="modal-body" id="mobile-content">
		<div class="row">
			<div class="col-12">
				<div class="input-group">
					<input type="text" class="form-control" placeholder='<?php echo e(__("all.layout.start_typing")); ?>' id="id" autocomplete="off">
				</div>
			</div>
		</div>
...
<?php if(auth()->user()->hasPermission('write-intervention')): ?>
<div class="modal" tabindex="-1" role="dialog" id="operator">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><?php echo e(__("all.layout.intervention")); ?></h5>
	  </div>
      <div class="modal-body" id="operator-content">
		<p><strong><?php echo e(__("all.layout.what_happened")); ?></strong></p>
        <div class="row">
			<div class="col-12">
				<input type="hidden" class="form-control" id="operator-wsid" value="">
				<input type="text" class="form-control" placeholder='<?php echo e(__("all.layout.event_description")); ?>' id="operator-event">
        		<a href="javascript:void(0)" id="save-intervention-suggestion" class="d-none"><i class="fas fa-save text-primary" title="<?php echo e(__('all.layout.save_this_suggestion')); ?>" style="position:absolute; top:10px; right:30px"></i></a>
			</div>
		</div>
     	<div class="row mt-2">
			<div class="col-6">
            	<strong><?php echo e(__("all.layout.operators")); ?></strong>
        		<br>
        		<div id="operators">
				<?php
            ...
            <div class="col-6">
            <label><input type="checkbox" name="special" value="keszenlet" id="operator-keszenlet"> <?php echo e(__("all.layout.standby_duty")); ?></label><br>
            <strong><?php echo e(__("all.layout.interval")); ?></strong><br>
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal"><?php echo e(__("all.button.close")); ?></button>
		<button type="button" class="btn btn-primary" id="create-operator-event" data-dismiss="modal"><?php echo e(__("all.button.send")); ?></button>
	  </div>
    </div>
  </div>
</div>
...
<div class="contextmenu dropdown-menu"></div>
  ...
  * html .ui-autocomplete {
    height: 170px;
		...
		<script src="<?php echo e(url('js/popper.1.14.3.min.js')); ?>"></script>
		<script src="<?php echo e(url('js/bootstrap.4.1.3.min.js')); ?>"></script>
    	<script src="<?php echo e(url('js/dark-mode-switch.min.js')); ?>"></script>
		<script type="text/javascript">
        	$(function() {
            	...
            	$("#operator-event").autocomplete({
                	...
                	var listItem = $("<li>")
        				.append("<div>" + item.label + "<i class='fas fa-trash-alt remove-intervention-suggestion text-danger float-right' title=\"<?php echo e(__('all.layout.delete_this_suggestion')); ?>\"></i></div>")
        				.appendTo(ul);
            	...
            	function refreshNotif() {
                	...
                	var getNotifications = $.post("<?php echo e(url('dashboard/payload')); ?>", payLoad, "JSONP");
            	...
            	function quickSearch(phrase) {
                   	var posting = $.post("<?php echo e(url('workstations/payload')); ?>", { '_token': $('meta[name=csrf-token]').attr('content'), action: 'quickSearch', phrase: phrase}, "JSONP");
        			posting.done(function(data) {
            			if (data.workstations.length == 0) {
                        	...
                        	for(var i = 0; i < data.workstations.length; i++) {
                            	...
                            	$("#search-result").append('<a href="' + '<?php echo e(url("/workstations")); ?>/' +   data.workstations[i].id + '" class="list-group-item list-group-item-action" data-ip="' + data.workstations[i].ip + '" data-id="' + data.workstations[i].id + '"  target="_blank">' + icon + " " + data.workstations[i].alias + ' (' + data.workstations[i].ip + ')</a>');	
                            }
            	...
            	$("body").contextmenu(function (event) {
                	...
                	if (clicked.hasClass("list-group-item")) {
                    	...
                    	<?php if(auth()->user()->hasPermission('write-intervention')): ?>
    						$(".contextmenu").append("<a href='javascript:' class='dropdown-item context-action' data-action='service' data-id='"+wsid+"'><?php echo e(__('all.layout.intervention')); ?></a>");
						<?php endif; ?>
                        
                        var posting = $.post("<?php echo e(url('workstations/payload')); ?>", { '_token': $('meta[name=csrf-token]').attr('content'), action: 'getWorkstationConnections', wsid: wsid}, "JSONP");
        				posting.done(function(data) {
            	...
            	$("#create-operator-event").on("click", function() {
                	...
                	if ($("#operator-keszenlet:checked").val() == 'keszenlet') {
                    	event = "(<?php echo e(__('all.layout.standby_duty')); ?>) " + event;
                    }
                	...
                	var posting = $.post("<?php echo e(url('workstations/payload')); ?>", { '_token': $('meta[name=csrf-token]').attr('content'), action: 'createOperatorEvent', wsid: wsid, event: event, operators: operators}, "JSONP");
        			posting.done(function(data) {
                    	$("#operator-wsid").val("");
                    	$("#operator-event").val("");
                    	$("#operator").modal("hide");
    			...
    			const redirectUrl = "<?php echo e(url('logout')); ?>";
    	...
    	<?php echo $__env->yieldContent('inject-footer'); ?>
    <style type="text/css">
...
<?php /**PATH /var/www/biglan/resources/views/layout.blade.php ENDPATH**/ ?>